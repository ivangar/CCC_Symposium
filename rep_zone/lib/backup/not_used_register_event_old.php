...
class Register
{
    ...
    private function validateEmpty()
    {
        ...
        if(!$this->validateCOI()){
            $this->feedback .= "Veuillez téléverser le formulaire de déclaration de conflits d'intérêts de la SCC *<br>";
            $empty = true;
        }
    ...
    WITHOUT PROGRAM START TIME

    private function registerAgenda()
    {

        $insert_query = "INSERT INTO agendas (arrrival_meal_time, QA_time, program_end_time) VALUES ( TIME( STR_TO_DATE( :arrrival_time, '%h:%i %p' ) ), TIME( STR_TO_DATE( :qa_time, '%h:%i %p' ) ), TIME( STR_TO_DATE( :end_time, '%h:%i %p' ) ) )";
        $query = $this->db_connection->prepare($insert_query);
     ...
     private function registerAgenda()
        {
            ...
            $query->execute(array(':arrrival_time'=>$this->event_data["arrrival_time"],
                                  ...
                                  ':qa_time'=>$this->event_data["qa_time"],
                                  ':end_time'=>$this->event_data["end_tim
                            ));
            $agenda_ID = $this->db_connection->lastInsertId();
