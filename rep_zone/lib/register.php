...
class Register
{
    ...
    public function IsFieldUnique($email)
    {
        ...
        while($result_row = $query->fetch() ){
            if($result_row['COUNT(*)'] == 0){
                ...
                echo "Ce courriel est déjà enregistré";
                return false;
            }
        }
    ...
    private function validateEmpty()
    {
        ...
        $validator->addValidation("rep_name","req","Le nom complet doit être fourni *");
        $validator->addValidation("rep_company","req","Le nom de la compagnie doit être fourni *");
        $validator->addValidation("rep_email","email","Un courriel valide doit être fourni *");
        $validator->addValidation("rep_email","req","Un courriel valide doit être fourni *");
        $validator->addValidation("rep_phone","req","Le numéro de téléphone doit être fourni *");
