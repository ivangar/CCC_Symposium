...
function GetData($data, &$program_section_id)
{
	...
	if (EmptyAnswer($questions_answers, $checkboxes)){
		return false;
	}
...
function EmptyAnswer($questions_answers, &$checkboxes){
	...
	foreach ($questions_answers as $qas_index => $qas_block) {
  	...
  	$checkboxes = $allQs; //keep count of the check boxes ticked
